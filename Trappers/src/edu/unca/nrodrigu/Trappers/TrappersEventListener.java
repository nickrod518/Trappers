		...
		if (event.getCause() == DamageCause.BLOCK_EXPLOSION) {
			...
			if (damagee instanceof Player) {
				...
				if(!(Boolean)plugin.getMetadata(player,"trapper", plugin)) {
					plugin.incScore(1);
					plugin.logger.info(player.getName() + " was hurt by a trap! That makes " + plugin.getScore() + " injuries.");
					player.sendMessage("Look out for traps! " + plugin.getScore() + " total reported casualties.");
				}
				else {
					player.sendMessage("Hurting a trapper doesn't count!");
				}
